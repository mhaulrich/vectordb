...
class AssetDatabase:
    ...
    def createVectorTable(self, name, dims, index_type):
        ...
        try:
            ...
            raise AssetDatabaseError("Failed to create table", e)
        finally:
            cursor.close()

    def deleteVectorTable(self, name):
        """Deletes a vector table in postgres.
        This removes the table and also removes it's metadata from the meta table."""
        cursor = self.cursor()
        try:
            # Delete table
            ...
            raise AssetDatabaseError("Failed to delete table", e)
        finally:
            cursor.close()
            self.commit()
    ...
    def tableExists(self, table_name):
        ...
        try:
            ...
            raise AssetDatabaseError("Failed to lookup tables", e)
            #return False
        finally:
            cursor.close()
            self.commit()
    ...
    def getDimensions(self, dbname):
        ...
        if dims is None:
            ...
            try:
                ...
                raise AssetDatabaseError("Failed to get dimensions", e)
            finally:
                cursor.close()
                self.commit()
        return dims
    ...
    def getExistingTables(self):
        ...
        try:
            ...
            for row in cursor:
                ...
                raise AssetDatabaseError("Failed to get existing tables", e)
        finally:
            cursor.close()
            self.commit()
        return tableNames
    ...
    def getNumberOfAssets(self, tablename):
        ...
        try:
            ...
            raise AssetDatabaseError("Failed to get number of assets", error)
        finally:
            cursor.close()
            self.commit()
        if not returnAsList:
    ...
    def insertVectorHashes(self, dbname, vector_hashes, assets):
        ...
        for i, vector_hash in enumerate(vector_hashes):
            ...
            try:
                ...
                cursor.close()
                raise AssetDatabaseError("Failed to get current hashes during insert of hashes", error)
        ...
        try:
            ...
            raise AssetDatabaseError("Failed to insert points to database", error)
        finally:
            cursor.close()
    ...
    def getAssets(self, dbname, vector_hash):
        ...
        try:
            ...
            raise AssetDatabaseError("Failed to lookup asset", error)
        finally:
            cursor.close()
            self.commit()
        return asset_ids
    ...
    def getPointsWithAsset(self, dbname, asset_id):
        ...
        try:
            ...
            raise AssetDatabaseError("Failed to lookup points for asset", error)
        finally:
            cursor.close()
            self.commit()
        return points
    ...
    def getSample(self, dbname, numRows, offset=0):
        ...
        try:
            ...
            raise AssetDatabaseError("Failed to lookup sample", error)
        finally:
            cursor.close()
            self.commit()
        return samples
